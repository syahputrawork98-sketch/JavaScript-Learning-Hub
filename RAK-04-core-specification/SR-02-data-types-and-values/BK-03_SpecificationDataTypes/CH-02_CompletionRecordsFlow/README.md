...
**Model Mental**:
- **Normal**: Lampu hijau (Terus).
- **Throw**: Lampu merah (Berhenti dan cari penangan).
- **Return**: Lampu kuning (Berhenti dan laporkan hasil ke atasan).
...
### Arsitek Mindset: Reliable Exception Handling
- Jangan gunakan `try-catch` secara berlebihan. Di level arsitektur, pengecualian (throw) adalah sinyal interupsi yang mahal. Desainlah sirkuit Anda agar mengembalikan status `normal` dengan kode error jika kegagalan tersebut adalah bagian dari aliran logika yang bisa diprediksi.
