...
**Model Mental**:
- **Undefined**: Kursi kosong yang belum dipesan.
- **Null**: Kursi yang sengaja dikosongkan untuk tamu yang tidak datang.
- **Boolean**: Saklar lampu (Nyala/Mati).
...
- Selalu lakukan pengecekan `null` dan `undefined` sebelum mengakses properti objek (gunakan **Optional Chaining** `?.`). Mengabaikan ketiadaan sinyal ini adalah penyebab utama "ledakan" runtime (TypeError) di Hub.
